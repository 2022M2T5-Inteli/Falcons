$(document).ready(function(){
    ...
    if (perguntas){
            ...
            console.log(options)
            perguntas = perguntas.filter(pergunta => pergunta.idEixo === 6);
            if (options){
...
$("#sizebutton").on('click', function(event) {
        ...
        if (window.localStorage.getItem(eixo2)) {
            window.localStorage.removeItem(eixo2)
        }
        ...
        window.location.replace(window.location.origin + "/formsEixo3")
